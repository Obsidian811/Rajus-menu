...
export async function fetchMenuFromGoogleSheet(sheetUrl: string): Promise<MenuItem[]> {
  try {
    const res = await fetch(
      "https://docs.google.com/spreadsheets/d/e/2PACX-1vQdPLFc73XvKGEZUYTGjJto0nfXBrt85rIDILZr1lil_-vr_RM_mFg5OgSpyRirX3zVKhNNOs8yy_AH/pub?output=csv&gid=0",
      { cache: "no-store" }
    );
    ...
    const parsed = (data as any[]).map((item) => ({
      ...
      language: item.language ? String(item.language).trim() : 'English',
    }));
